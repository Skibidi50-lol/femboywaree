...
local ThemeManager = loadstring(game:HttpGet("https://pastefy.app/j4jgLATI/raw"))()
...
local Window = Library:CreateWindow({
	Title = "FemboyWare - Survive Zombie Arena",
	Center = true,
	AutoShow = true,
	Resizable = true,
        ...
        if Config.VoidShardFarm then
            ...
            if folder then
                ...
                for _, shard in ipairs(folder:GetChildren()) do
                    if not Config.VoidShardFarm then
                        break
                    end

                    if shard.Name == "VoidShardPart" and shard:IsA("BasePart") then
                        hrp.CFrame = shard.CFrame + Vector3.new(0, 3, 0)
                        task.wait(0.2)
                    end
                end
...
AutoBox:AddToggle("AutoVoidShard", {
    Text = "Void Shard Farm",
    Default = false,
    Callback = function(v)
        Config.VoidShardFarm = v
    end,
})
...
local WatermarkConnection = game:GetService("RunService").RenderStepped:Connect(function()
	...
	if CanDoPing then
		Library:SetWatermark(("github.com/Skibidi50-lol | %d fps | %d ms"):format(
			math.floor(FPS),
			GetPing()
		));
	else
		Library:SetWatermark(("github.com/Skibidi50-lol | %d fps"):format(
			math.floor(FPS)
		));
	end
end);
...
ThemeManager:SetFolder("FEMBOYPICS")
SaveManager:SetFolder("FEMBOYPICS/SZA")
SaveManager:SetSubFolder("SZA") -- if the game has multiple places inside of it (for example: DOORS) 
